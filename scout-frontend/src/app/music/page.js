...
import MusicForm from '../music_form';
...
function MusicPageContent() {
  ...
  const fetchMusicItems = useCallback(async () => {
    ...
    try {
      const response = await axios.get(`${API_CONTENT_BASE_URL}/music/?${params.toString()}`, {
        headers: token ? { 'Authorization': `Bearer ${token}` } : {}
      });
      setMusicItems(Array.isArray(response.data) ? response.data : (response.data.results || []));
    } catch (err) {
  ...
  const handleDelete = async (itemId) => {
    if (window.confirm('Are you sure you want to delete this music entry?')) {
      ...
      const token = localStorage.getItem('accessToken');
      try {
        await axios.delete(`${API_CONTENT_BASE_URL}/music/${itemId}/`, {
          headers: token ? { 'Authorization': `Bearer ${token}` } : {}
        });
